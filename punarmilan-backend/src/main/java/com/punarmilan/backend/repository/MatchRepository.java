...
public interface MatchRepository extends JpaRepository<Match, Long> {
    ...
    @Query("SELECT m FROM Match m WHERE " +
           "(m.user1 = :user AND (SELECT p.city FROM Profile p WHERE p.user = m.user2) = :city) OR " +
           "(m.user2 = :user AND (SELECT p.city FROM Profile p WHERE p.user = m.user1) = :city)")
    Page<Match> findMatchesByCity(@Param("user") User user, 
                                 @Param("city") String city, 
                                 Pageable pageable);
    
    @Query("SELECT m FROM Match m WHERE " +
           "(m.user1 = :user AND (SELECT p.age FROM Profile p WHERE p.user = m.user2) BETWEEN :minAge AND :maxAge) OR " +
           "(m.user2 = :user AND (SELECT p.age FROM Profile p WHERE p.user = m.user1) BETWEEN :minAge AND :maxAge)")
    Page<Match> findMatchesByAgeRange(@Param("user") User user,
